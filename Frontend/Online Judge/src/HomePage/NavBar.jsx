...
function NavBar() {
  ...
  useEffect(() => {
    ...
    axios.get(`${URL}/api/auth/profile`, { withCredentials: true })
      .then(response => {
        setUser({email : response.data.user.username, id : response.data.user.id })
  ...
  return (
    <div className='flex justify-between w-[120%] bg-slate-800 border-b-2 border-b-red-400 border-t-4 h-[3rem] items-center p-4 m-0'>
        ...
        <ul className=' flex gap-3'>
          {user ? (<>
            <li onClick={handleUserVisibility} className='relative'><img src="/user.png" alt="" className='h-[25px] w-[25px] mr-10 cursor-pointer'/></li>
            {showUser && (
              <div className='h-[10rem] w-auto border-2 rounded-md bg-blue-200 absolute z-20 p-2' style={{ right: '30px', top: '20%', transform: 'translateY(-50%)' }} >
          ...
          (
            <>
              <li><NavLink to={`/login`} state={{from : location}} replace>Login</NavLink></li>
              {/* <li><Navigate to={'/login'} state={{from:location}} replace>Login</Navigate></li> */}
